...
export default function ProfileDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "chats" | "favorites">(
    "profile",
  );
    ...
    try {
      ...
      if (conversations) {
        ...
        const favoriteIds = new Set(
          favorites?.map((f) => f.conversation_id) || [],
        );
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!publicUrl) {
        alert("Failed to get public URL for photo");
        setUploading(false);
        return;
      }
  ...
  const handleSaveProfile = async () => {
    try {
      if (!formData.full_name || !formData.phone_number || !formData.city) {
        alert("Please fill in all required fields (Name, Phone, City)");
        return;
      }
