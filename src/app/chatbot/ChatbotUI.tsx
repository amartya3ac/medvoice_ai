...
// PDF export handled via native browser print dialog
...
export default function ChatbotUI({
  ...
  const getFilteredDoctors = (specialty: string, city: string) => {
    ...
    let doctors = GLOBAL_DOCTOR_REGISTRY.filter(
      (d) =>
        (d.specialty.toLowerCase().includes(spec) ||
          spec.includes(d.specialty.toLowerCase())) &&
        d.city === city,
    );
    ...
    if (doctors.length === 0) {
      doctors = GLOBAL_DOCTOR_REGISTRY.filter(
        (d) =>
          (d.specialty.toLowerCase().includes(spec) ||
            spec.includes(d.specialty.toLowerCase())) &&
  ...
  const getSpecialtyIconLocal = (specialty: string) => {
    ...
    if (s.includes("dermatologist") || s.includes("skin"))
      return { icon: Sparkles, color: "text-blue-400" };
    if (s.includes("cardiologist") || s.includes("heart"))
      return { icon: Heart, color: "text-red-400" };
    if (s.includes("neurologist") || s.includes("brain"))
      return { icon: Brain, color: "text-purple-400" };
    if (s.includes("orthopedic") || s.includes("bone") || s.includes("joint"))
      return { icon: Bone, color: "text-orange-400" };
    if (
      s.includes("ent specialist") ||
      s.includes("ear") ||
      s.includes("nose") ||
      s.includes("throat")
    )
      return { icon: Ear, color: "text-cyan-400" };
    if (
      s.includes("gastroenterologist") ||
      s.includes("stomach") ||
      s.includes("liver")
    )
      return { icon: Stethoscope, color: "text-emerald-400" };
    if (s.includes("pulmonologist") || s.includes("lung"))
      return { icon: Activity, color: "text-sky-400" };
    if (s.includes("nephrologist") || s.includes("kidney"))
      return { icon: Droplets, color: "text-indigo-400" };
    if (s.includes("urologist") || s.includes("urine"))
      return { icon: Wind, color: "text-blue-500" };
    if (s.includes("oncologist") || s.includes("cancer"))
      return { icon: ShieldAlert, color: "text-rose-400" };
    if (s.includes("pediatrician") || s.includes("child"))
      return { icon: Baby, color: "text-pink-400" };
    if (s.includes("gynecologist") || s.includes("female"))
      return { icon: Flower2, color: "text-rose-300" };
    if (s.includes("obstetrician") || s.includes("pregnancy"))
      return { icon: HeartHandshake, color: "text-pink-300" };
    if (s.includes("endocrinologist") || s.includes("hormone"))
      return { icon: Thermometer, color: "text-yellow-400" };
    if (s.includes("ophthalmologist") || s.includes("eye"))
      return { icon: Eye, color: "text-blue-300" };
    if (s.includes("psychiatrist") || s.includes("mental"))
      return { icon: BrainCircuit, color: "text-teal-400" };
    if (s.includes("psychologist") || s.includes("behavior"))
      return { icon: Smile, color: "text-teal-300" };
    if (s.includes("dentist") || s.includes("teeth"))
      return { icon: Zap, color: "text-slate-100" };
    if (s.includes("surgeon") && !s.includes("ortho"))
      return { icon: Scissors, color: "text-slate-400" };
    if (s.includes("physician") || s.includes("general"))
      return { icon: Stethoscope, color: "text-blue-400" };
    if (s.includes("radiologist") || s.includes("x-ray"))
      return { icon: Scan, color: "text-slate-500" };
    if (s.includes("anesthesiologist"))
      return { icon: Syringe, color: "text-indigo-300" };
    if (s.includes("pathologist") || s.includes("biopsy"))
      return { icon: Microscope, color: "text-rose-500" };
    if (s.includes("hematologist") || s.includes("blood"))
      return { icon: Droplets, color: "text-red-600" };
    if (s.includes("rheumatologist") || s.includes("arthritis"))
      return { icon: Activity, color: "text-orange-500" };
    if (s.includes("sexologist"))
      return { icon: Heart, color: "text-pink-500" };
    if (s.includes("allergist") || s.includes("immune"))
      return { icon: Wind, color: "text-green-300" };
    if (s.includes("veterinarian"))
      return { icon: Smile, color: "text-amber-600" };
    if (s.includes("homeopathic"))
      return { icon: Leaf, color: "text-green-400" };
    if (s.includes("ayurvedic"))
      return { icon: Sprout, color: "text-green-600" };
    if (s.includes("emergency"))
      return { icon: AlertTriangle, color: "text-red-500" };
    if (s.includes("family medicine"))
      return { icon: Home, color: "text-blue-300" };
    if (s.includes("infectious"))
      return { icon: ShieldCheck, color: "text-green-500" };
    if (s.includes("geriatrician") || s.includes("elderly"))
      return { icon: UserPlus, color: "text-slate-300" };
    if (s.includes("neonatologist"))
      return { icon: Baby, color: "text-pink-200" };
    if (s.includes("sports medicine"))
      return { icon: Zap, color: "text-yellow-500" };
    if (s.includes("pain management"))
      return { icon: CloudRain, color: "text-slate-400" };
    if (s.includes("pharmacologist"))
      return { icon: Pill, color: "text-blue-400" };
    if (s.includes("geneticist"))
      return { icon: Dna, color: "text-purple-500" };
    if (s.includes("sleep medicine"))
      return { icon: Moon, color: "text-indigo-400" };
    if (s.includes("cardiothoracic") || s.includes("cardiac surgeon"))
    ...
    if (s.includes("maxillofacial"))
      return { icon: UserIcon, color: "text-slate-400" };
    if (s.includes("palliative") || s.includes("comfort"))
      return { icon: HeartHandshake, color: "text-emerald-300" };
    if (s.includes("occupational medicine"))
      return { icon: Briefcase, color: "text-orange-300" };
    if (s.includes("reproductive"))
      return { icon: Dna, color: "text-rose-200" };
    if (s.includes("interventional"))
      return { icon: Target, color: "text-blue-500" };
    if (s.includes("transplant"))
      return { icon: HeartPulse, color: "text-rose-600" };
    if (s.includes("bariatric") || s.includes("weight"))
      return { icon: Scale, color: "text-indigo-300" };
    if (s.includes("audiologist") || s.includes("hearing"))
      return { icon: Volume2, color: "text-blue-400" };
    if (s.includes("speech"))
      return { icon: MessageSquare, color: "text-blue-300" };
    if (s.includes("critical care") || s.includes("intensivist"))
      return { icon: Activity, color: "text-red-700" };
    if (s.includes("occupational therapist"))
      return { icon: Handshake, color: "text-orange-400" };
    if (s.includes("physiatrist") || s.includes("rehabilitation"))
      return { icon: Accessibility, color: "text-emerald-400" };
    if (s.includes("tropical"))
      return { icon: Palmtree, color: "text-amber-500" };
    if (s.includes("biochemist"))
      return { icon: FlaskConical, color: "text-cyan-500" };
    if (s.includes("internal medicine"))
      return { icon: Stethoscope, color: "text-blue-400" };
    if (s.includes("uro-oncologist"))
      return { icon: ShieldAlert, color: "text-rose-500" };
    if (s.includes("robotic surgeon"))
  ...
  const chat = useChat({
    ...
    onError: (err: Error) => {
      ...
      setStructuredAnalysis({
        ...
        diagnosis:
          "Connection to the Diagnostic Engine could not be established. Please ensure the local backend is running on `http://127.0.0.1:8000` via `python backend/main.py`.",
        home_remedies: [],
        medical_treatments: [],
        doctors: [],
      });
        ...
        setMessages([
          {
            ...
            role: "assistant",
            content:
              "Hello! I am MedVoice AI, your Senior Diagnostic Specialist. I am connected and ready to provide clinical-grade analysis. How can I assist you today?",
          },
        ]);
      }
    };
    ...
    try {
      ...
      const mapRemedy = (item: any) => {
        ...
        return {
          ...
          description:
            item.description || item.details || "Step-by-step instruction",
          clinical_logic:
            item.clinical_logic || item.logic || "Standard medical protocol.",
        };
      };
      ...
      return {
        ...
        diagnosis:
          data.diagnosis ||
          data.analysis ||
          "High-precision evaluation complete.",
        home_remedies: Array.isArray(home) ? home.map(mapRemedy) : [],
        medical_treatments: Array.isArray(medical)
          ? medical.map(mapRemedy)
          : [],
        doctors:
  ...
  const latestAssistantMessage = messages
    .filter((m: any) => m.role === "assistant" && m.id !== "welcome")
    .pop();

  useEffect(() => {
    if (latestAssistantMessage) {
      console.log(
        "TRACE: latestAssistantMessage update detected:",
      ...
      if (
        symptomText.toLowerCase().includes("headache") &&
        raw.home_remedies.length === 0
      ) {
        raw.home_remedies = [
          {
            title: "Cold Compress",
            description: "Apply a cold pack to your forehead for 15 minutes.",
            clinical_logic:
              "Vasoconstriction reduces blood flow and inflammation.",
          },
          {
            title: "Hydration",
            description: "Drink 500ml of water immediately.",
            clinical_logic:
              "Dehydration is a primary trigger for primary headaches.",
          },
          {
            title: "Dark Room",
            description: "Rest in a quiet, dark room for 30 minutes.",
            clinical_logic: "Reduces sensory input and photophobia symptoms.",
          },
        ];
  ...
  const { icon: DynamicSpecialtyIcon, color: specialtyIconColor } =
    getSpecialtyIconLocal(structuredAnalysis.specialty);
  ...
  const analysisText =
    ...
    !latestAssistantMessage.content.includes("{")
      ...
      : "");
  const hasRemedies =
    structuredAnalysis.home_remedies?.length > 0 ||
    structuredAnalysis.medical_treatments?.length > 0;
  const hasAnalysis = !!(
    analysisText ||
    hasRemedies ||
    isAnalyzing ||
    latestAssistantMessage
  ); // NUCLEAR TRIGGER: Show if anything is happening
  ...
  useEffect(() => {
    ...
    hasAnalysis,
    structuredAnalysis.home_remedies,
    structuredAnalysis.medical_treatments,
    treatmentTab,
  ]);
  ...
  const exportToPDF = async () => {
    try {
      ...
      if (!element) {
        alert("Report content not found. Please generate an analysis first.");
      ...
      printStyle.innerHTML = `
        @media print {
          ...
          #pdf-export-content * { display: block !important; }
          #pdf-export-content { display: block !important; }
          ...
          .bg-slate-950 { background: #f9f9f9 !important; }
      ...
      if (iframeDoc) {
        ...
        iframeDoc.write(`
          ...
          <head>
            ...
            <title>MED-VOICE Diagnostic Report</title>
            <style>
              * { margin: 0; padding: 0; box-sizing: border-box; }
              body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
              .content { padding: 20mm; background: white; color: black; line-height: 1.6; }
              h1, h2, h3, h4 { margin: 15px 0 10px 0; color: #000; }
              h1 { font-size: 24px; }
              h2 { font-size: 18px; border-bottom: 2px solid #0066cc; padding-bottom: 8px; }
              h3 { font-size: 14px; }
              .card { margin: 15px 0; padding: 12px; border: 1px solid #ddd; background: #f9f9f9; }
              .title { font-weight: bold; color: #000; }
              .description { color: #333; font-size: 12px; margin-top: 5px; }
              .logic { color: #0066cc; font-size: 10px; margin-top: 8px; font-style: italic; }
              @page { margin: 10mm; }
  ...
  const handleSend = async (
    ...
    const textToAnalyze =
      (customSymptomText !== undefined ? customSymptomText : symptomText) || "";
  ...
  return (
    <div className="flex w-full min-h-screen bg-slate-950 overflow-hidden">
      ...
      <div
        ref={uiContainerRef}
        className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 pb-10 pt-0 relative"
      >
        {/* Header Section */}
        <div className="flex flex-col items-center mb-16 space-y-8 anim-item -mt-6">
          ...
          <div className="text-center space-y-6 max-w-4xl">
            ...
            <p className="text-slate-400 text-lg md:text-xl font-medium leading-relaxed max-w-2xl mx-auto">
              Describe your symptoms naturally using voice or text, or upload a
              prescription for instant, personalized medical guidance powered by
          ...
          <header className="w-full max-w-6xl bg-slate-950/40 border border-white/10 rounded-[32px] p-2 flex flex-col md:flex-row items-center justify-between shadow-2xl backdrop-blur-3xl ring-1 ring-white/5 no-print gap-4 md:gap-0">
            ...
            <div className="flex items-center gap-4 px-6 border-b md:border-b-0 md:border-r border-white/10 w-full md:w-auto py-4 md:py-0 h-auto md:h-10">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-black text-slate-100 tracking-tight leading-none truncate max-w-[150px]">
                  {user?.email || "aishikaroy2003@gmail.com"}
                </span>
            ...
            <div className="flex items-center gap-4 px-6 md:px-10 border-b md:border-b-0 md:border-r border-white/10 w-full md:w-auto py-4 md:py-0 h-auto md:h-10">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] font-black text-slate-500 uppercase tracking-tighter mt-1">
                  Backend: Connected
                </span>
              </div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 relative w-full items-start max-w-7xl mx-auto px-0 md:px-4">
          ...
          <div className="space-y-8">
            ...
            <div className="glass-card bg-slate-900/60 border border-white/5 shadow-2xl rounded-3xl p-6 md:p-8 anim-item">
              <h2 className="text-xl font-black text-white flex items-center gap-3 mb-8 uppercase tracking-tighter">
                ...
                Upload Prescription{" "}
                <span className="text-[10px] text-slate-500 normal-case ml-2">
                  (Optional)
                </span>
              </h2>
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    <span className="text-purple-400">Click to upload</span> or
                    drag and drop
                  </p>
          ...
          <div
            ...
            className="space-y-8 print:bg-slate-950 print:p-10"
          >
            ...
            <div className="hidden print-only-header flex-col items-center mb-10 border-b-2 border-blue-500/30 pb-8">
              ...
              <div className="text-[8px] text-slate-500 mt-4 uppercase">
            ...
            <div className="glass-card bg-slate-900/60 border border-white/5 shadow-2xl rounded-3xl p-6 md:p-8 anim-item flex flex-col min-h-[400px] md:min-h-[500px]">
              <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
                <h2 className="text-xl font-black text-white flex items-center gap-3 uppercase tracking-tighter">
                  <DynamicSpecialtyIcon
                    className={cn("w-6 h-6", specialtyIconColor)}
                  />
                  AI Analysis
                </h2>
                <button
                ...
                >
                  <div
                    className={cn(
                      "w-1.5 h-1.5 rounded-full",
                      autoListen ? "bg-blue-400 animate-pulse" : "bg-slate-600",
                    )}
                  />
              ...
              <div className="flex-1 flex flex-col">
                ...
                !(isAnalyzing || localIsAnalyzing) ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-600">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest italic opacity-40">
                            ...
                            .map((line: string, i: number) => {
                              ...
                              if (trimmedLine.startsWith("### "))
                                return (
                                  <h3
                                    key={i}
                                    className="text-lg font-black text-white mt-4 border-b border-white/5 pb-2 uppercase tracking-tighter"
                                  >
                              ...
                              if (trimmedLine.startsWith("**")) {
                                ...
                                if (boldMatch)
                                  return (
                                    <p key={i}>
                                      <span className="text-white font-black text-[12px] tracking-tight mr-1">
                                        {boldMatch[1]}
                                      </span>
                                      {boldMatch[2]}
                                    </p>
                              ...
                              if (trimmedLine.startsWith("- "))
                                return (
                                  <li
                                    key={i}
                                    className="ml-4 list-disc marker:text-emerald-500"
                                  >
            ...
            {(hasAnalysis || isAnalyzing || localIsAnalyzing) && (
              <div className="glass-card bg-slate-900/60 border border-white/5 shadow-2xl rounded-3xl p-8 anim-item">
                ...
                <div className="flex bg-slate-950/80 p-1.5 rounded-2xl mb-8 border border-white/5 w-full max-w-[340px] no-print mx-auto md:mx-0">
                  <button
                    onClick={() => setTreatmentTab("home")}
                    className={`flex-1 py-2.5 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest ${treatmentTab === "home" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "text-slate-500 hover:text-slate-300"}`}
                  >
                    Home Remedies
                  </button>
                  <button
                    onClick={() => setTreatmentTab("medical")}
                    className={`flex-1 py-2.5 text-[10px] font-black rounded-xl transition-all uppercase tracking-widest ${treatmentTab === "medical" ? "bg-slate-900 text-slate-400 shadow-lg shadow-indigo-500/20" : "text-slate-500 hover:text-slate-300"}`}
                  >
                    Medical Treatment
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  {(treatmentTab === "home"
                    ...
                    <div
                      key={idx}
                      className="bg-slate-950/40 border border-white/5 rounded-2xl p-6 hover:bg-slate-900/60 transition-all border-l-4 border-l-blue-500/30"
                    >
                      <h4 className="text-[14px] font-black text-slate-100 uppercase tracking-tight mb-2">
                        {item.title}
                      </h4>
                  ...
                  {(
                    ...
                    <p className="text-center text-slate-600 py-10 font-bold uppercase tracking-widest text-[10px]">
                      No specific remedies suggested yet.
                    </p>
                  )}
                </div>
              </div>
            ...
            {(hasAnalysis || isAnalyzing || localIsAnalyzing) && (
              <div className="glass-card bg-slate-900/60 border border-white/5 shadow-2xl rounded-3xl p-6 md:p-8 anim-item">
                ...
                <div className="space-y-6">
                  {(structuredAnalysis.doctors || []).map(
                    (doc: any, idx: number) => (
                      <div
                        key={idx}
                        className="bg-slate-950/60 border border-white/5 rounded-3xl p-8 hover:bg-slate-900 transition-all group relative overflow-hidden ring-1 ring-white/5"
                      >
            ...
            <div className="hidden print-remedies flex-col gap-10">
              <div className="space-y-6">
                <h3 className="text-2xl font-black text-blue-400 uppercase border-b border-blue-500/30 pb-2">
                  Home Remedies
                </h3>
                {structuredAnalysis.home_remedies?.map(
                  (item: any, idx: number) => (
                    <div
                      key={idx}
                      className="p-6 bg-slate-900/40 border border-white/10 rounded-2xl"
                    >
                      ...
                      {item.clinical_logic && (
                        <div className="mt-4 pt-4 border-t border-white/5">
                          ...
                          <p className="text-[10px] text-emerald-400/80 italic mt-1">
                            {item.clinical_logic}
                          </p>
                        </div>
                      )}
              ...
              <div className="space-y-6">
                <h3 className="text-2xl font-black text-purple-400 uppercase border-b border-purple-500/30 pb-2">
                  Medical Treatments
                </h3>
                {structuredAnalysis.medical_treatments?.map(
                  (item: any, idx: number) => (
                    <div
                      key={idx}
                      className="p-6 bg-slate-900/40 border border-white/10 rounded-2xl"
